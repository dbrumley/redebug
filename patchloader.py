...
class PatchLoader(object):
    ...
    def traverse(self, patch_path):
        ...
        if os.path.isfile(patch_path):
            ...
            common.verbose_print(f'  [-] {patch_path}: {magic_type}')
            ...
            for root, dirs, files in os.walk(patch_path):
                for file in files:
                    ...
                    common.verbose_print(f'  [-] {file_path}: {magic_type}')
                    if magic_type.startswith('text'):
                        main_type, sub_type = magic_type.split('/')
                        self._process(file_path)
        self._npatch = len(self._patch_list)
        elapsed_time = time.time() - start_time
        print(f'[+] {self._npatch} patches ... {elapsed_time:.1f}s\n')
        return self._npatch
            ...
            if line.startswith('--- '):
                if diff_vuln_lines:
                    ...
                    if len(diff_norm_lines) >= common.ngram_size:
                        common.verbose_print(f'      {diff_file} {diff_cnt} (ext: {magic_ext})')
                        ...
                        common.verbose_print(f'      {diff_file} {diff_cnt} (ext: {magic_ext}) - skipped ({len(diff_norm_lines)} lines)')
                    diff_vuln_lines.clear()
    ...
    def _normalize(self, patch, ext):
        ...
        if ext == common.FileExt.C or ext == common.FileExt.Java or ext == common.FileExt.CPP:
    ...
    def _get_file_type(self, file_path):
        ...
        if file_type is None:
            ...
            elif sub_type.startswith('x-c++'):
                magic_ext = common.FileExt.CPP
