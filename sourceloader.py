...
class SourceLoader:
    ...
    def traverse(self, source_path, patch):
        ...
        if os.path.isfile(source_path):
            ...
            common.verbose_print(f'  [-] {source_path}: {magic_type}')
        ...
        print(f'[+] {self._nmatch} possible matches ... {elapsed_time:.1f}s\n')
        return self._nmatch
    ...
    def _normalize(self, source, ext):
        ...
        if ext in [common.FileExt.C, common.FileExt.CPP, common.FileExt.Java]:
            norm_lines = []
    ...
    def _query_bloomfilter(self, source_norm_lines, magic_ext):
        ...
        if len(source_norm_lines) < common.ngram_size:
            common.verbose_print(f'      - skipped ({len(source_norm_lines)} lines)')
            return False
        ...
        for i in range(0, num_ngram):
            if num_ngram_processed > common.bloomfilter_size/common.min_mn_ratio:
                common.verbose_print(f'      - split Bloom filters ({num_ngram_processed} n-grams)')
                for patch_id in range(0, self._npatch):
                    if magic_ext == self._patch_list[patch_id].file_ext:
                        ...
                        if is_match:
                            ...
                            common.verbose_print(f'      - match (patch #{patch_id} : source #{self._nsource})')
                            self._nmatch += 1
                num_ngram_processed = 0
                self._bit_vector.setall(0)
